...
namespace input_360
{
    ...
    {
        SettingsFile settings = new SettingsFile("input_360");
        InputThresholds thresholds = new InputThresholds() { TriggerLeft = 40, TriggerRight = 40 };
        bool connected = false;
        Controller controller;
        Thread stateThread, buttonThread;
        bool[] buttonStates;
        ...
        bool testHomeButton()
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                stat = secret_get_gamepad(i, out controllerState);

                if (stat != 0)
                    continue;

                value = ((controllerState.wButtons & 0x0400) != 0);
        ...
        public string Name
        {
            get
            {
                return "WoWmapper Xbox 360 plugin";
            }
        }
